...
`/quota` 조회에 성공한 계정의 5시간·주간 잔여량이 **둘 다 0보다 크면** 그 계정에
남아 있는 사용량 제한 차단을 해제합니다. 호스트가 차단을 기록하는 두 곳(로그인
저장소의 계정 항목과 자격 증명 풀 상태 파일)을 호스트의 잠금 API로 함께 정리하므로,
차단이 한쪽에만 남아 계정이 계속 막히는 일이 없습니다.

해제 대상은 조회한 그 계정으로 한정되며, 다음은 절대 건드리지 않습니다.

- `auth_error`·`account_disabled` 차단 — 재로그인으로만 풀려야 하는 상태입니다.
- 조회가 실패했거나 5시간·주간 창이 응답에 없는 계정.
- 잔여량이 0인 계정, 그리고 조회 중 취소된 실행.
- 조회 도중 자격 증명이나 차단 상태가 바뀐 계정 — 최신 상태를 되돌리지 않습니다.
- 계정 풀 API가 없는 구버전 호스트 — 이때는 기존처럼 조회만 합니다.

잔여량이 남아 있다는 것이 순간적인 요청 제한까지 사라졌다는 뜻은 아니므로, 이
동작은 “다시 시도해도 되는 계정의 차단을 치워 주는” 정책입니다. 상태 기록에
실패하면 사용량 출력은 그대로 유지하고 다음 문구만 덧붙입니다.
