...
- 5시간·주간 잔여량이 모두 0보다 큰 계정은 그 계정의 사용량 제한(rate limit) 차단을
  자동으로 해제 (인증 오류·계정 비활성화 차단은 그대로 유지)
...
헤더 뒤의 `- blocked`는 그 계정을 호스트가 지금 거부하고 있다는 뜻이며, 차단된
계정에만 붙습니다. 자세한 규칙은 아래 “사용량 제한 차단 동기화”를 참고하세요.
...
`/quota` 조회에 성공하면 그 계정의 차단 상태를 방금 읽은 잔여량에 맞춥니다. 호스트가
차단을 기록하는 두 곳(로그인 저장소의 계정 항목과 자격 증명 풀 상태 파일)을 호스트의
잠금 API로 함께 고치므로, 차단이 한쪽에만 남아 계정이 계속 막히는 일이 없습니다.

- 응답에 있는 5시간·주간 잔여량이 **모두 0보다 크면** 그 계정에 남아 있는 사용량
  제한 차단을 해제합니다. 5시간 제한이 없는 요금제(예: ChatGPT Pro Lite)는 주간 창만
  응답하므로 주간 잔여량만으로 판단합니다.
- 응답에 있는 5시간·주간 중 **하나라도 0%면** 차단이 없거나 이미 만료된 계정에 사용량 제한
  차단을 기록합니다. 차단 종료 시각은 소진된 창의 재설정 시각이고, 응답에 재설정
  시각이 없으면 1분, 재설정이 너무 멀면 호스트와 같은 상한인 2일로 맞춥니다.
- 잔여량은 출력에 보이는 값 그대로(반올림 후) 판단하므로, 화면에 0%로 보이는 계정이
  차단되지 않은 채 남지 않습니다.

이미 차단된 계정은 다시 기록하지 않습니다. 차단된 계정은 출력에서 머리글 뒤에
표시가 붙습니다.
...
동기화 대상은 조회한 그 계정으로 한정되며, 다음은 절대 건드리지 않습니다.

- `auth_error`·`account_disabled` 차단 — 재로그인으로만 풀려야 하는 상태입니다.
- 조회가 실패했거나 5시간·주간 창이 하나도 응답에 없어 판단할 근거가 없는 계정.
- 조회 중 취소된 실행.
- 조회 도중 자격 증명이나 차단 상태가 바뀐 계정 — 최신 상태를 되돌리지 않습니다.
- 계정 풀 API가 없는 구버전 호스트 — 이때는 기존처럼 조회만 하고, 차단 여부를
  알 수 없으므로 표시도 붙이지 않습니다.

잔여량이 남아 있다는 것이 순간적인 요청 제한까지 사라졌다는 뜻은 아니므로, 해제는
“다시 시도해도 되는 계정의 차단을 치워 주는” 정책이고, 기록은 “이미 소진된 계정을
다음 세션이 다시 찔러 보지 않게 하는” 정책입니다. 상태 기록에 실패하면 사용량
출력은 그대로 유지하고 다음 문구만 덧붙입니다.
